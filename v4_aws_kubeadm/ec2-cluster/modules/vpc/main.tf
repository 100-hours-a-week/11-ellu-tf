...
resource "aws_subnet" "private_k8s" {
  count                   = length(var.availability_zones)
  vpc_id                  = aws_vpc.main.id
  cidr_block              = "10.1.${count.index * 4 + 1}.0/24"
  availability_zone       = var.availability_zones[count.index]

  tags = {
    Name                                        = "${var.cluster_name}-private-k8s-subnet-${count.index + 1}"
    "kubernetes.io/cluster/${var.cluster_name}" = "shared"
...
resource "aws_subnet" "private_rds" {
  count             = length(var.availability_zones)
  vpc_id            = aws_vpc.main.id
  cidr_block        = "10.1.${count.index * 4 + 2}.0/24"
  availability_zone = var.availability_zones[count.index]
  
  tags = {
    Name = "${var.cluster_name}-private-rds-subnet-${count.index + 1}"
  }
}
...
resource "aws_route_table_association" "private_k8s" {
  count          = length(aws_subnet.private_k8s)
